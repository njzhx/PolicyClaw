    ...
    try:
        ...
        for script in scripts:
            if script.string and '<record>' in script.string:
                record_soup = BeautifulSoup(script.string, 'html.parser')
                records = record_soup.find_all('record')
                break

        if not records:
            print('[ERROR] 江苏省粮食和物资储备厅信息公开爬虫：未找到数据记录')
        ...
        for record in records:
            try:
                ...
                # 处理URL
                if href.startswith('/'):
                    article_url = "https://lsj.jiangsu.gov.cn" + href
                ...
                try:
                    ...
                    # 尝试多个选择器
                    for selector in ['.TRS_Editor', '.zoom', '#zoom', '.content', '#content', '.article-content', '.main-txt', 'div[class*="content"]']:
        ...
        print(f'[OK] 江苏省粮食和物资储备厅信息公开爬虫：成功抓取 {len(policies)} 条前一天数据')
        ...
        print(f'[ERROR] 江苏省粮食和物资储备厅信息公开爬虫：抓取失败 - {e}')
        print("----------------------------------------")
...
def save_to_supabase(data_list):
    try:
        ...
        return save_to_policy(data_list, "江苏省粮食和物资储备厅_信息公开")
    except Exception:
        return data_list
...
def run():
    try:
        ...
        print(f'[ERROR] 江苏省粮食和物资储备厅信息公开爬虫：运行失败 - {e}')
        print("----------------------------------------")
        return []
